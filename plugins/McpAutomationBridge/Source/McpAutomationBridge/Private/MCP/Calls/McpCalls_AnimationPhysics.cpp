...
{
	B.Array(TEXT("artifacts"), TEXT("cleanup: asset paths to delete; existing assets are force-deleted, missing paths count as already clean."))
...
static void S_ConfigureVehicle(FMcpSchemaBuilder& B)
{
	B.String(TEXT("actorName"), TEXT("Name of the actor."))
	 ...
	 .Number(TEXT("mass"), TEXT("configure_vehicle/configure_physics_body: mass override in kg (vehicle: must be > 0; physics body: must be >= 0.001)."))
	 .Number(TEXT("dragCoefficient"), TEXT("configure_vehicle: aerodynamic drag coefficient; also applied as linear damping on the root primitive."))
...
static void S_PlayMontage(FMcpSchemaBuilder& B)
{
	B.String(TEXT("actorName"), TEXT("Name of the actor."))
	 ...
	 .Number(TEXT("playRate"), TEXT("play_montage: playback rate multiplier (default 1)."))
...
static void S_SetBoneKey(FMcpSchemaBuilder& B)
{
	B.String(TEXT("assetPath"), TEXT("Existing asset to modify (e.g. the montage for add_montage_section)."))
	 ...
	 .Integer(TEXT("frame"), TEXT("set_bone_key/set_curve_key/add_notify/add_sync_marker: frame index at the asset's sampling frame rate (default 0)."))
	 .Bool(TEXT("save"), TEXT("Persist the created/modified asset to disk (default true; most authoring actions)."))
...
{
	B.String(TEXT("assetPath"), TEXT("Existing asset to modify (e.g. the montage for add_montage_section)."))
	 .String(TEXT("curveName"), TEXT("set_curve_key: float curve to key (created when missing if createIfMissing)."))
...
{
	B.String(TEXT("assetPath"), TEXT("Existing asset to modify (e.g. the montage for add_montage_section)."))
	 .String(TEXT("notifyName"), TEXT("add_notify/add_notify_state/add_montage_notify: notify event name, also written to the notify object's NotifyName so PlayMontageNotify name-matching fires (one of notifyName/notifyClass required); bind_animation_notify: notify whose AnimNotify_<name> event is created and bound."))
...
static void S_CreateMontage(FMcpSchemaBuilder& B)
{
	B.String(TEXT("name"), TEXT("Name identifier."))
	 ...
	 .String(TEXT("skeletonPath"), TEXT("Asset path (e.g., /Game/Path/Asset)."))
	 .String(TEXT("path"), TEXT("Legacy alias of savePath (create_procedural_animation, create_pose_library, create_montage, create_blend_space_1d/2d, create_aim_offset, create_animation_blueprint, create_control_rig, create_ik_rig, create_ik_retargeter) or of skeletonPath (create_skeleton)."))
	 .String(TEXT("slotName"), TEXT("create_montage/add_montage_slot: montage slot track name, renamed or created as needed (default DefaultSlot); add_slot_node: anim slot for the new Slot node (stored as '<groupName>.<slotName>')."))
	 .Bool(TEXT("save"), TEXT("Persist the created/modified asset to disk (default true; most authoring actions)."))
	 .Number(TEXT("length"), TEXT("Length in seconds (create_animation_sequence, set_sequence_length, create_montage)."))
	 .Required({TEXT("name"), TEXT("skeletonPath")});
}
...
{
	B.String(TEXT("assetPath"), TEXT("Existing asset to modify (e.g. the montage for add_montage_section)."))
	 .String(TEXT("sectionName"), TEXT("add_montage_section: name for the new section; set_section_timing: existing section to retime."))
	 .Number(TEXT("startTime"), TEXT("Section start time in seconds (alias of 'time')."))
	 .String(TEXT("montagePath"), TEXT("play_montage: montage to play; add_montage_section: montage to edit ('assetPath'/'name' accepted as fallback)."))
	 .String(TEXT("name"), TEXT("Name identifier."))
	 .Number(TEXT("time"), TEXT("add_montage_section: section start time in seconds (alias of startTime); add_montage_notify: notify trigger time in seconds (default 0); set_blend_in/set_blend_out: alias of blendTime in seconds."))
	 .Bool(TEXT("save"), TEXT("Persist the created/modified asset to disk (default true; most authoring actions)."))
...
static void S_CreateBlendSpace1D(FMcpSchemaBuilder& B)
{
	B.String(TEXT("name"), TEXT("Name identifier."))
	 ...
	 .String(TEXT("skeletonPath"), TEXT("Asset path (e.g., /Game/Path/Asset)."))
	 .String(TEXT("path"), TEXT("Legacy alias of savePath (create_procedural_animation, create_pose_library, create_montage, create_blend_space_1d/2d, create_aim_offset, create_animation_blueprint, create_control_rig, create_ik_rig, create_ik_retargeter) or of skeletonPath (create_skeleton)."))
	 .String(TEXT("axisName"), TEXT("create_blend_space_1d: blend axis name (default Speed); set_axis_settings: new axis display name (unchanged when omitted)."))
...
static void S_AddStateMachine(FMcpSchemaBuilder& B)
{
	B.String(TEXT("blueprintPath"), TEXT("Anim Blueprint asset path (bind_animation_notify, add_state_machine, add_state, add_transition, set_transition_rules; 'assetPath' accepted for the state-machine actions)."))
	 ...
	 .Number(TEXT("positionX"), TEXT("Graph node X coordinate."))
	 .Number(TEXT("positionY"), TEXT("Graph node Y coordinate."))
	 .Bool(TEXT("save"), TEXT("Persist the created/modified asset to disk (default true; most authoring actions)."))
...
{
	B.String(TEXT("blueprintPath"), TEXT("Anim Blueprint asset path (bind_animation_notify, add_state_machine, add_state, add_transition, set_transition_rules; 'assetPath' accepted for the state-machine actions)."))
	 .String(TEXT("stateName"), TEXT("add_state: name for the new state node (engine validation may adjust it)."))
	 .String(TEXT("machineName"), TEXT("Alias of stateMachineName."))
	 .String(TEXT("assetPath"), TEXT("Existing asset to modify (e.g. the montage for add_montage_section)."))
	 .String(TEXT("stateMachineName"), TEXT("State machine name for add_state_machine/add_state/add_transition/set_transition_rules."))
	 .Number(TEXT("positionX"), TEXT("Graph node X coordinate."))
