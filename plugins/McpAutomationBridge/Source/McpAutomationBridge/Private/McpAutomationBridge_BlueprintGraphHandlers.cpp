...
// Each manage_blueprint graph subAction is a HandleBlueprintGraph* member,
// dispatched directly by the classed actions (MCP/Calls/McpCalls_ManageBlueprint.cpp).
// The members share MCP_BPGRAPH_PREAMBLE (blueprint + target-graph resolution)
// and the file-scope arrange/classify/lookup helpers defined below.
#if WITH_EDITOR
// =============================================================================
...
#if WITH_EDITOR
// Shared preamble for the extracted Blueprint-graph action members below.
// _HEAD validates payload + any provided paths (blueprint-free actions);
// the full macro then resolves the blueprint and target graph, exposing
...
#endif // WITH_EDITOR (Blueprint-graph preamble)

// Node/pin lookup shared by the graph action members (hoisted from the
// retired dispatcher's per-request lambdas).
#if WITH_EDITOR
static UEdGraphNode *FindNodeByIdOrName(UEdGraph *TargetGraph, const FString &Id) {
    if (Id.IsEmpty()) {
      return nullptr;
    }
...
#undef MCP_BPGRAPH_PREAMBLE
#undef MCP_BPGRAPH_PREAMBLE_HEAD
