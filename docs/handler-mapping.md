...
| `manage_ai` | — (BehaviorTree/Navigation survive as `ManageAI()` union inputs for schema-union validation only) | CLASSED: `FMcpCall` instances @ MCP/Calls/McpCalls_ManageAi.cpp (dispatched from the call registry; implementations are the `HandleAi*` members @ AIHandlers.cpp for the 43 core actions, `HandleBehaviorTree*` @ BehaviorTreeHandlers.cpp for the 7 Behavior Tree graph actions, and the `HandleNavigation*` wrappers @ NavigationHandlers.cpp for the 12 navigation actions) |
