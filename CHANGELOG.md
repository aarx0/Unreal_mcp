...
### Legacy registration-map dispatch path deleted (2026-07-10)

- **`FMcpCallRegistry` is the sole dispatch.** With every family classed, the old handler-registry path is gone end-to-end — `RegisterHandler`, `InitializeHandlers`, the `AutomationHandlers` map, `MCP_REGISTER_HANDLER`, and the `RegisterDecls` hook are deleted along with every symbol orphaned behind them. `ProcessAutomationRequest` looks up `(tool, payload action)` in the call registry and runs `FMcpCall::Execute`; a miss falls through to `UNKNOWN_ACTION`. There is no secondary dispatch chain. (`df9a2fcc`)

### manage_blueprint fully de-monolithed — the last route dispatchers extracted (2026-07-09)

- **The one delegation-wired family is now direct-dispatch like the other twenty.** `manage_blueprint` was classed delegation-wired (2026-07-05) because its four route dispatchers had external/recursive callers; those bodies are now hoisted into per-action members and the dispatchers deleted — `HandleBlueprintAction`, `HandleBlueprintGraphAction`, `HandleSCSAction`, `HandleManageWidgetAuthoringAction`, and `HandleCommonUiAction` are gone. Each class calls its member directly (`MCP_BP_CORE_CALL` / `MCP_BP_GRAPH_CALL` / `MCP_BP_CUI_CALL` / `MCP_BP_WIDGET_CALL`). The Niagara authoring monolith `HandleManageNiagaraAuthoringAction` was extracted to per-action `HandleNiagara*` members the same day; `HandleBlueprintModifyScs` is the one member kept live for its external caller (EditorFunctionHandlers.cpp), and `HandleNiagaraGraphAction` survives as a shared graph handler. (`2a081181` + `e6b6a68a` / `62d9f4a8` / `9e76a3e7` / `4a56d839` / `d80bcf3e` / `5694a2a5`)

### Silent-success convention complete (2026-07-09)

- **Fail-in-place bag-setter convention, lint-green.** `McpHandlerUtils::FMcpWriteReport` (per-field `MarkApplied`/`MarkFailed`) + `SendWriteReportResponse` (`McpResponseHelpers.h`) own the one success rule: nothing requested → `NO_CHANGES_REQUESTED`; any field failed → `success=false` + `PROPERTY_WRITE_FAILED` with `appliedProperties[]` + `failed[]` (no rollback — applied fields stay); every field applied → success. `tests/schema/silent-success-lint.ps1` is the forcing function — it fails until every multi-optional-field "bag" setter is converted or justified; the allowlist holds only 10 atomic-setter/creator false positives, and the combat/interaction scaffolds were removed outright rather than carved out. (`a34fce4c` … `a44421d8`)

### Strict property importer (2026-07-08/09)

- **One importer, no silent coercion.** The two rival `ApplyJsonValueToProperty` implementations were unified into a single strict importer (the `helpers.h` twin deleted, and its read-direction sibling `ExportPropertyToJsonValue` too): bool←bool and number←number only, range-checked ints, strict struct import, kind-checked writes — the coercion paths are dead. The `McpBridge.PropertyImport.*` automation spec (Accept / Reject / KindAgreement / ObjectPath) locks the behavior. (`cf4d90ef` / `ae5e1739` / `64804fce` / `93aca7e6`)

### manage_blueprint classed — twenty-first (final) FMcpCall family (2026-07-05)

- **145 classes in `MCP/Calls/McpCalls_ManageBlueprint.cpp` (31 Core + 14 BlueprintGraph + 92 WidgetAuthoring + 8 CommonUi) — the twenty-first and FINAL classed family, completing the FMcpCall classing migration.** Unlike the other twenty families, this one is DELEGATION-wired: no string dispatcher was retired and no branch body was extracted, because all four route dispatchers have callers beyond the registration lambda and cannot die — `HandleBlueprintAction` is called externally by EditorFunctionHandlers.cpp and recurses into `HandleBlueprintGraphAction` (:4999,:5020) and `HandleSCSAction` (:5790). Each class `Run()` delegates to its route's surviving dispatcher with EXACTLY the args the retired lambda passed (byte-behaviour-identical for canonical inputs): CommonUi -> `HandleCommonUiAction` (Action arg `manage_common_ui`), WidgetAuthoring -> `HandleManageWidgetAuthoringAction` (`manage_widget_authoring`), BlueprintGraph -> `HandleBlueprintGraphAction` and Core -> `HandleBlueprintAction` (both `manage_blueprint`) — each dispatcher gates on that Action arg and reads the real sub-action from the payload, so a routed request resolves exactly as it did through the lambda. The registration lambda, the `IsCommonUiAction`/`IsWidgetAuthoringAction`/`IsBlueprintGraphAction` predicates (their only caller), and `McpDecl_ManageBlueprint.h` died with the conversion; the four route dispatchers were widened private->public so the classes can delegate, and the `ManageBlueprintCore`/`BlueprintGraph`/`WidgetAuthoring`/`CommonUi` lists plus the routing-table row survive for boot schema-union validation.
- **Zero decl fixes — a pure verbatim port.** The 140 param arrays were copied byte-identical from the retired shim rows (`P_ManageBlueprint_N` -> `P_<Action>`; the five zero-param bind_*/create_property_binding actions pass `{}`); the surviving dispatchers still own every payload read, so the shim contracts reconciled clean. Flags: `RequiresEditor` on Core + BlueprintGraph (BlueprintHandlers.cpp and BlueprintGraphHandlers.cpp are whole-body `#if WITH_EDITOR`), NOT on WidgetAuthoring (WidgetAuthoringHandlers.cpp has no editor gate) or CommonUi (CommonUIHandlers.cpp gates on `MCP_HAS_COMMON_UI`, not `WITH_EDITOR`) — flagging either would newly reject the GEditor-less runs the shim served (networking/asset-Texture precedent); `Mutating` on every action except the 18 pure `get_*`/`list_*`/`preview_*`/`probe_*` reads.
- **One hidden name deleted and ledgered:** the transport-dead `get_nodes` else-if in BlueprintGraphHandlers.cpp (:1790-1859) — never advertised, no decl row, no manufactured literal — removed and the chain re-spliced (connect_pins -> break_pin_links, brace-balance net 0); the only handler-body edit. Advertise candidate parked for Aaron (dead-name sweep). Hidden-entry counter 2 -> 1.
- **FMcpCall classing migration COMPLETE — all 21 families classed.** Every tool now dispatches through the `FMcpCall` registry and `McpRegisterAllActionDecls()` is a no-op. manage_blueprint is the sole delegation-wired family (its dispatchers survive); the remaining follow-up is the module split that de-members the surviving handler bodies off the subsystem.
- **Gate (statics only — build/boot/live pending):** action-decl lint 1116/allowlist 123, param reconciliation 1428/1278 (allowlist 73, stale 0 — no allowlist change), docs-reference green; byte-verified — the 140 param arrays are byte-identical to the retired shim rows, all 145 macro delegations carry the correct per-route Action-arg literal + flags, the get_nodes excision is brace/paren-balanced net 0, and every touched TU's brace/#if balance matches HEAD. NOT yet compiled or boot-validated.

### manage_asset classed — twentieth FMcpCall family (2026-07-05)

- **122 classes in `MCP/Calls/McpCalls_ManageAsset.cpp` (43 Core + 58 MaterialAuthoring + 21 Texture); all three string dispatchers are gone** — the registration lambda that split MaterialAuthoring off to `HandleManageMaterialAuthoringAction` and Texture off to `HandleManageTextureAction` died with it, along with `HandleAssetAction` (the Core dispatcher); the `IsMaterialAuthoringAction`/`IsTextureAction` predicates died with their only caller, the manufactured `manage_material_authoring`/`manage_texture` gate literals died with the dispatchers, and the routed lists plus `ManageAssetCore` survive only for boot schema-union validation. Core actions keep their existing AssetWorkflow/AssetQuery members (`HandleAssetAction` only ever delegated); two arms converted to typed calls per rule 5 — `save_all` calls the control_editor member `HandleControlEditorSaveAll` directly, `create_render_target` calls the new texture member `HandleTextureCreateRenderTarget` directly; `save`'s file-static `HandleSaveAssetStatic` gained a thin `HandleSaveAsset` member wrapper. MaterialAuthoring's 58 actions extracted verbatim from the retired if/else chain to `HandleMaterial*` members (each replicating the chain's whole-body `#if WITH_EDITOR` / "Editor only." EDITOR_ONLY stub; the shared expression macros hoisted to file scope with `MCP_NODE_ID` deliberately left un-undef'd for the arrange_graph helpers downstream; the three prologue alias remaps and four OR-branches became members shared by their advertised actions via a passed action literal). Texture's 21 actions + `create_render_target` became per-action file statics behind `HandleTexture*` members that apply the retired response wrapper's conversion via one shared `SendTextureResult` static (audio precedent). The dispatcher declarations, the handler-map registration, and `McpDecl_ManageAsset.h` died with the conversion; the 14 `McpSafeAssetSave` tails (6 AssetWorkflow + 1 Material add_landscape_layer + helper-routed + Texture) ride inside the extracted bodies unchanged.
- **Eight shadowed-dead AssetWorkflow arms deleted with their members** — the Core dispatcher re-implemented eight MaterialAuthoring-owned names in unreachable copies (create_material, create_material_instance, add_material_node, connect_material_pins, remove_material_node, break_material_connections, get_material_node_details, rebuild_material): MaterialAuthoring is tested first, so the arms and their members could never execute (animation_physics precedent). Also deleted: the hidden `source_control_enable` arm+member (transport-dead advertise candidate), six hidden material branches (add_component_mask, add_dot_product, add_cross_product, add_desaturation, add_append, set_cast_shadows), and six hidden texture branches (import_texture, set_texture_filter, set_texture_wrap, create_cube_texture, create_volume_texture, create_texture_array) that `IsMaterialAuthoringAction`/`IsTextureAction` never routed to. Ledgered in the dead-name sweep; recover from git. Deleting these branches orphaned two advertised tool-schema params whose only readers were the removed code — `luminanceFactors` (McpTool_ManageAsset.cpp — read only by the hidden add_desaturation) and `targetPin` (McpTool_ManageAudio.cpp — read only by the shadowed HandleConnectMaterialPins). Both are kept in the published schema so this classing commit holds the golden byte-identical (pure refactor), exempted in the param-reconciliation allowlist, and parked as advertise-or-remove candidates for Aaron — shrinking the published surface is a separate reviewable change, not smuggled into the refactor.
- **Decl burn-down — 13 rows fixed with body evidence:** the five own-branch shadowed rows re-derived from the LIVE MaterialAuthoring bodies (dropped the dead copies' spellings — create_material's `properties`, create_material_instance's `parameters`, add_material_node's materialPath/posX/posY/value/color/texturePath, remove_material_node's and get_material_node_details's materialPath/expressionIndex, all proven absent from the live bodies); the three alias rows (connect_material_pins/break_material_connections/rebuild_material) re-derived from their remap targets (connect_nodes/disconnect_nodes/compile_material), dropping materialPath/fromExpression/toExpression/targetPin, materialPath/expressionIndex/inputName, and materialPath respectively; the five data-table rows' assetPath/dataTablePath/tablePath/path alias group flipped all-required→optional (McpLoadDataTableArg joint-rejects only when all are absent, rowName stays required on add/edit/remove), plus import_data_table's sourceText/csv/json/content source aliases. Flags: `RequiresEditor` on the 43 Core + 58 Material actions (editor-gated TUs), NOT on the 21 Texture actions + `create_render_target` (TextureHandlers.cpp has no editor gate — networking precedent, flagging would newly reject GEditor-less runs); `Mutating` on everything except the 25 pure reads.
- **Gate:** action-decl lint 1116/allowlist 123, param reconciliation 1428/1278 (allowlist 73, stale 0 — the two orphaned params added to the allowlist, and `provider` pruned since the manage_asset deletions removed its last reader), docs-reference green; published schema byte-identical to golden (live); byte-verified — extracted Material and Texture bodies are byte-identical to their HEAD branch bodies (sliced directly, no dedent), untouched regions of every touched TU carry the same brace/paren/#if–#endif balance as HEAD, and the 13 param-row fixes are enumerated with the dropped spelling per row. Build clean (after a one-line self-containment fix: deleting the manage_asset decl header reshuffled the unity blobs and exposed McpAutomationBridge_GASHandlers.cpp using APlayerController::GetPawn without including GameFramework/PlayerController.h — same latent class as the combat/inventory fixes); boot validation 1116 decls + routing passed; published schema byte-identical to golden (live); live smoke green (Core reads, the create_render_target typed Texture edge + create, the data-table dataTablePath-alias flip, get_material_node_details reaching the live MaterialAuthoring owner, and source_control_enable/import_texture/add_desaturation enum rejects); ui-nav exact baseline.

### manage_audio classed — nineteenth FMcpCall family (2026-07-05)

- **50 classes in `MCP/Calls/McpCalls_ManageAudio.cpp`; both string dispatchers are gone** — `HandleAudioAction` (AudioHandlers.cpp, an 18-prefix entry gate over a ~1,900-line if/else chain) and the `HandleManageAudioAuthoringAction` wrapper + its static `HandleAudioAuthoringRequest` chain (AudioAuthoringHandlers.cpp, 27 blocks), plus the registration lambda that split them on `IsAudioAuthoringAction` (the predicate died with its only caller, the manufactured `manage_audio_authoring` gate literal died with the wrapper, and `WithPayloadSubAction` died with the lambda — the audio lambda was its last caller). The 23 live core bodies extracted verbatim to `HandleAudio*` members (whitespace-only dedent), each replicating the chain's `#else` NOT_IMPLEMENTED stub ("Audio actions require editor build"); `fade_sound_in`/`fade_sound_out` share `HandleAudioFadeSoundInternal` with the chain's `Lower == TEXT("fade_sound_in")` comparisons resolved to a `bFadeIn` parameter and the `action` echo resolved to the two literals, and `fade_sound`'s lowercased echo inlined to its literal (the only non-verbatim body deltas). The 27 authoring actions became per-action file statics — block bodies verbatim from the retired chain, every block's return-only exits verified — behind `HandleAudioAuthoring*` members that apply the retired wrapper's response conversion via one shared `SendAudioAuthoringResult` static (verbatim, including its BuildErrorResponse "code"-field mapping) and replicate its `#else` EDITOR_REQUIRED stub per member. The chains' `Lower`/`SubAction` prologues and payload-null checks died (`FMcpCall::Execute` owns payload-null); the dispatcher declarations, the handler-map registration, and `McpDecl_ManageAudio.h` died with the conversion; the routing row and `AudioAuthoring()`/`ManageAudioCore()` lists stay for boot schema-union validation. The 21 `McpSafeAssetSave` tails (11 core + 10 authoring) ride inside the extracted bodies unchanged — including the create_sound_class/create_sound_mix device-registration hang documented in TODO.md, which is asset-type-specific and travels with the verbatim bodies.
- **Seven shadowed-dead arms deleted with their dedicated members** — the core dispatcher re-implemented seven AudioAuthoring-owned names in unreachable copies (create_dialogue_voice, create_dialogue_wave, set_dialogue_context, create_reverb_effect, create_source_effect_chain, add_source_effect, create_submix_effect): the registration lambda always routed those names to the authoring chain first, so the arms and their members (`HandleCreateDialogueVoice`, `HandleCreateDialogueWave`, `HandleSetDialogueContext`, `HandleCreateReverbEffect`, `HandleCreateSourceEffectChain`, `HandleAddSourceEffect`, `HandleCreateSubmixEffect`, ~560 lines) could never execute (animation_physics precedent). Ledgered in the dead-name sweep (new AudioHandlers section; the sweep's 17 already-executed audio_* alias-arm entries annotated as pre-classing coordinates); recover from git. Ten stale reconciliation pins pruned with them (chainName, chainPath, contextIndex, effectName, lateGain, pluralization, reflectionsGain, voiceName, voicePath, waveName).
- **Decl burn-down — 14 of 50 rows fixed:** the seven shadowed actions' rows re-derived from the live authoring blocks (the fleet had unioned the dead copies' spellings into the rows): dropped voiceName/outputPath/pluralization (create_dialogue_voice), waveName/soundPath/outputPath (create_dialogue_wave), wavePath/voicePath/contextIndex (set_dialogue_context), effectName/outputPath/reflectionsGain/lateGain (create_reverb_effect), chainName/outputPath (create_source_effect_chain), chainPath/effectName (add_source_effect — its effectType flipped required→optional, the live block only consults it when effectPresetPath is absent), effectName/outputPath (create_submix_effect). play_sound_2d and play_sound_at_location dropped the functionName/params/path spellings no body reads (bootstrap residue). The mixName/mix/name (+soundClassName/soundClass) alias chains on set_sound_mix_class_override, clear_sound_mix_class_override, and set_base_sound_mix flipped all-required→all-optional (first-present-wins fallback chains; the bodies reject only at asset resolution), as did fade_sound's soundName/actorName pair and create_audio_component's soundPath/path pair (joint reject only when both are absent) — create_audio_component's String-typed volume/pitch verified handler-true against the body's TryGetStringField + Atof reads, not a kind bug. The other 36 rows are byte-identical to the retired shim rows.
- **Gate (statics only — build/boot/live pending):** action-decl lint 1116/allowlist 123, param reconciliation 1428/1281 (allowlist 72, stale 0), docs-reference green; byte-verified — all 21 dedent-only core bodies reconstruct their HEAD branches exactly under the inverse re-indent, all 27 authoring block bodies are byte-identical substrings of the new file, the wrapper conversion and both non-editor stubs carried over byte-exact (Self-> prefix and per-member replication aside), 36 of 50 param rows byte-identical to the retired shim rows with the 14 fixes enumerated row-by-row, and brace/paren/#if–#endif balance deltas are net 0 per touched file. NOT yet compiled or boot-validated.

### manage_geometry classed — eighteenth FMcpCall family (2026-07-05)

- **76 classes in `MCP/Calls/McpCalls_ManageGeometry.cpp`; the string dispatcher is gone** — `HandleGeometryAction` (GeometryHandlers.cpp, 87 flat arms) already delegated every branch to a dedicated static free function in the same TU, so no body moved: the 76 members are `HandleGeometry*` thin wrappers (level-structure precedent), each replicating the chain's `MCP_HAS_FULL_GEOMETRY_SCRIPT` `#else` stub per member ("GeometryScript operations require UE 5.1 or later", NOT_SUPPORTED). The TU is compiled only `WITH_EDITOR` — retired dispatcher included — so the wrappers live inside the same whole-file gate and non-editor builds omit them exactly as they omitted the chain, with the baked `RequiresEditor` flag answering at `Execute()` (animation_physics authoring/skeleton precedent: no non-editor stub existed to replicate). `inset`/`outset` keep the shared `HandleInsetOutset` static with its boolean resolved per member; `generate_lods` keeps `HandleGenerateLODsGeometry`; the chain's `subAction` prologue, the dispatcher declaration, the handler-map registration, and `McpDecl_ManageGeometry.h` died with the conversion; the routing row and `ManageGeometry()` list stay for boot schema-union validation. The three `McpSafeAssetSave` tails (generate_lods / set_lod_settings / set_lod_screen_sizes) ride inside the untouched statics.
- **Eleven hidden raw-mesh DynamicMesh CRUD arms deleted with their statics** (~800 lines: create_procedural_mesh, append_triangle, append_vertex, delete_vertex, delete_triangle, get_vertex_position, set_vertex_position, set_uvs, set_vertex_color, split_normals, translate_mesh) — the dead-name-sweep ledger's geometry advertise-candidate family, transport-dead since the enum gate landed (none in `ManageGeometry()`, no decl rows, no manufactured literals; each static's only caller was its dispatcher arm). Dispositions updated in the ledger; advertise candidate parked for Aaron — recover from git. Six stale reconciliation pins pruned with them (enableCollision, groupID, setAll, splitAngle, triangleIndex, vertexIndex). Hidden-entry counter 31 → 20.
- **Zero decl fixes — the first fully clean port of a large family:** all 76 rows re-verified against the static bodies. Read-sets reconcile on all 76 (create_plane's `height`/`widthSegments`/`heightSegments` are the body's first-choice reads with `depth`/`widthSubdivisions`/`depthSubdivisions` fallbacks, not dead spellings), and every required flag matches a body reject: the joint rejects (targetActor AND toolActor on the three booleans, actorName AND trimActorName on boolean_trim, actorName AND splineActorName on duplicate_along_spline, each separately on extrude_along_spline) are AND requirements, not one-of alias pairs; `get_mesh_info` and `generate_lods` joint-reject only when BOTH actorName and assetPath are absent (both stay optional, at-least-one handler-enforced); `sweep`'s `splineActorName` stays optional — absent falls back to a straight-path sweep. `RequiresEditor` baked on all 76; `Mutating` on everything except the one reader, `get_mesh_info`.
- **Gate (statics only — build/boot/live pending):** action-decl lint 1116/allowlist 123, param reconciliation 1428/1297 (allowlist 82, stale 0), docs-reference green; schema snapshot PASS against the running editor (21 tools — that editor still runs the pre-classing binary, and this classing changes no schema surface, so golden is expected to hold after a rebuild too); byte-verified — untouched regions of GeometryHandlers.cpp are byte-identical to HEAD, all 76 wrapper delegations match their retired arms 1:1, all 76 param rows byte-identical to the retired shim rows, brace/paren/#if counts net 0. NOT yet compiled or boot-validated.

### animation_physics classed — seventeenth FMcpCall family (2026-07-05)

- **85 classes in `MCP/Calls/McpCalls_AnimationPhysics.cpp`; all three string dispatchers are gone** — `HandleAnimationPhysicsAction` (AnimationHandlers.cpp, the ~3,860-line monolith with 46 branches), the `HandleManageAnimationAuthoringAction` wrapper + its static `HandleAnimationAuthoringRequest` chain (AnimationAuthoringHandlers.cpp, 46 blocks serving 47 names), and `HandleManageSkeleton` (SkeletonHandlers.cpp, 40 branches serving 41 names), plus the registration lambda that split them on `IsAnimationAuthoringAction`/`IsSkeletonAction` (both predicates died with their only caller, and the `manage_animation_authoring`/`manage_skeleton` internal gate literals died with the dispatchers). The 12 live core bodies extracted verbatim to `HandleAnimPhys*` members (whitespace-only dedent), each replicating the chain's shared `Resp`/`bSuccess`/`Message`/`ErrorCode` prologue, its response tail (default-message fill + `SendAutomationResponse`), and its `#else` NOT_IMPLEMENTED stub ("Animation/Physics actions require editor build."), with the lowercased `LowerSub` echoes inlined to the member's action literal; `play_montage` keeps its typed forward to the live `HandlePlayAnimMontage` handler (the internal `play_anim_montage` literal survives inside that handler). The 42 authoring actions became per-action file statics — block bodies verbatim from the retired chain, every block's depth-0 terminal `return Response;` verified — behind `HandleAnimAuthoring*` members that apply the retired wrapper's response conversion via one shared `SendAnimAuthoringResult` static (verbatim); `set_blend_in`/`set_blend_out` share one static with the chain's `SubAction == "set_blend_in"` ternary resolved to a `bBlendIn` parameter (the only non-verbatim body delta). The 29 skeleton actions became `HandleSkeleton*` members: 21 thin wrappers over the dedicated members the retired chain delegated to (which keep their own per-member editor gates), 8 verbatim extractions of its inline branches (create_skeleton, add_bone, remove_bone, set_bone_parent, set_vertex_weights, auto_skin_weights, copy_weights, mirror_weights). The authoring and skeleton members live inside their TUs' whole-file `#if WITH_EDITOR` gates exactly like the retired dispatchers (no non-editor stub existed to replicate); the chains' subAction prologues (including HandleManageSkeleton's MISSING_ACTION reject — the transport mirrors action↔subAction before dispatch, so the field is always present), the three dispatcher declarations, the handler-map registration, and `McpDecl_AnimationPhysics.h` all died with the conversion. `McpTool_AnimationPhysics.cpp`'s stale "(55 actions)" header comment reads 85 now (Core 14 + AnimationAuthoring 42 + Skeleton 29, disjoint by boot validation).
- **setup_ragdoll / activate_ragdoll are advertised-but-unwired (Aaron's dogfood find), preserved exactly:** both names sit in `AnimationPhysicsCore()`, so they always fell through to the primary dispatcher's terminal else — `"Animation/Physics action '<name>' not implemented"`, NOT_IMPLEMENTED. Their classes are explicit stubs replicating that response byte-for-byte (flags `None`, so non-editor builds still answer the chain's own `#else` stub rather than the flag's generic message). The orphaned `HandleSetupRagdoll`/`HandleActivateRagdoll` implementations (no call sites — both fully written, actor-resolving, physics-blending handlers) are KEPT unchanged as the wire-up candidates; wire-the-orphans (restores the advertised capability) vs retire-the-rows (schema change, golden regen) is logged for Aaron in TODO.md. Their two decl rows are byte-identical to the retired shim rows.
- **Shadowed-dead deletion — the heaviest yet:** the primary dispatcher re-implemented THIRTY of the 42 routed authoring actions (create_animation_sequence, set_sequence_length, add_bone_track, set_bone_key, set_curve_key, add_notify, create_montage, add_montage_section, add_montage_slot, set_section_timing, add_montage_notify, set_blend_in, set_blend_out, link_sections, create_blend_space_1d, create_blend_space_2d, add_blend_sample, set_axis_settings, set_interpolation_settings, create_aim_offset, add_aim_offset_sample, add_state_machine, add_state, add_transition, set_transition_rules, add_blend_node, add_cached_pose, add_slot_node, create_control_rig, create_ik_rig) — all unreachable since the registration lambda routed every one of those names to the authoring sibling first (each verified ∈ `AnimationAuthoring()`); all 30 copies deleted. Hidden transport-dead branches deleted with them and ledgered (docs/dead-name-sweep-2026-07-04.md): the primary's NOT_SUPPORTED rig family (add_control, add_rig_unit, connect_rig_elements, add_ik_chain — no decl row, not in any routing list, schema enum rejects the names), the authoring TU's same four rig names (circular WRONG_HANDLER_ROUTE stubs) plus its dead `create_pose_library` copy (the name is in `AnimationPhysicsCore()`, so the primary's implementation is the live one), and SkeletonHandlers' twelve hidden unadvertised names — set_physics_asset, remove_physics_body, get_physics_asset_info, set_morph_target_value, list_morph_targets, delete_morph_target, delete_socket/remove_socket, get_bone_transform, list_virtual_bones, delete_virtual_bone (ten dedicated members + branches, the sweep's advertise-candidate read/delete family, parked for Aaron) and the inline `preview_physics` echo stub. Hidden-entry counter 51 → 31; all recoverable from git.
- **Decl burn-down — 27 of 85 rows fixed, backed by the bodies' own reads and rejects:** (1) dropped spellings whose only reader was a deleted shadowed copy — `sequenceName` (create_animation_sequence), `montageName` (create_montage), add_notify's `animationPath`+`time`, set_bone_key's `time`+`position`, set_curve_key's `time`, add_blend_sample's `sampleX`/`sampleY`, set_axis_settings' `axisIndex`/`gridNum`, set_interpolation_settings' `interpolationSpeed`, add_state's `animationPath`/`isEntry`/`isExit`, add_transition's and set_transition_rules' `sourceState`/`targetState` (+`condition`), add_blend_node's `nodeType`, add_cached_pose's `poseName`, create_control_rig's `savePath`, create_ik_rig's `savePath`+`meshPath`, create_aim_offset's `is1D`, create_pose_library's `save` — plus bootstrap residue nothing ever read (the min/max/grid spellings on create_aim_offset and create_blend_space_1d/2d) and create_anim_blueprint's `meshPath` (last reader deleted at 2026-07-04i); (2) required→optional where the live body joint-rejects an alias group or serves a default — the blueprintPath/assetPath pairs (add_state, add_state_machine, add_transition, set_transition_rules), create_state_machine's blueprintPath/name, play_montage's montagePath/assetPath, add_montage_section's assetPath/montagePath/name triple, setup_retargeting's assets/retargetAssets, the notifyClass/notifyName one-ofs (add_notify, add_montage_notify), create_anim_blueprint's savePath (path fallback, `/Game/Blueprints` default), create_control_rig's skeletonPath (consulted only when skeletalMeshPath is absent; rig creation proceeds with neither); (3) optional→required from the live bodies' own rejects — add_notify's `assetPath` (sole remaining asset spelling) and set_section_timing's `startTime` (explicit MISSING_TIME reject). The other 58 rows are byte-identical to the retired shim rows. 24 stale reconciliation pins pruned with the deleted bodies (allowlist 112 → 88).
- Flags authored: `RequiresEditor` on 83 of 85 (all three retired chains whole-editor-gated) — NOT on the two ragdoll stubs (exact-preservation, see above); `Mutating` on the 72 writers — unflagged are the readers `get_animation_info`, `get_skeleton_info`, `list_bones`, `list_sockets`, `list_physics_bodies`, the honest-error/no-write actions `import_morph_targets`, `normalize_weights`, `prune_weights`, `auto_skin_weights`, `copy_weights`, `assign_cloth_asset_to_mesh`, and the two ragdoll stubs. Gate battery: decl lint PASS (1116 decls, allowlist 123), param reconciliation PASS (1428 declared / 1305 payload-read / allowlist 88, stale 0), docs-reference PASS, per-file brace/paren/#if-#endif balance net 0 (heaviest preprocessor nesting of the migration — the 25+ feature macros ride inside the moved bodies unchanged), param-row byte-compare vs the retired shim rows (58 identical, 27 documented fixes), member-body byte-compare vs the retired branches (12 core + 8 skeleton whitespace-only dedents verified by inverse re-indent; 41 authoring blocks verbatim; 21 skeleton wrapper delegations). Build + live smoke pending the next editor session.

### manage_ai classed — sixteenth FMcpCall family (2026-07-05)

- **61 classes in `MCP/Calls/McpCalls_ManageAi.cpp`; all three string dispatchers are gone** — `HandleManageAIAction` (AIHandlers.cpp, the ~3,810-line monolith whose top-of-function routers forwarded BehaviorTree/Navigation names to the sibling dispatchers), `HandleBehaviorTreeAction` (BehaviorTreeHandlers.cpp, 7 actions), and `HandleManageNavigationAction` (NavigationHandlers.cpp, 12 delegating branches), plus the file-static `IsManageAI*SubAction` router wrappers and their only-caller predicates `IsBehaviorTreeAction`/`IsNavigationAction` (McpConsolidatedActionRouting.h). The 37 advertised inline bodies extracted verbatim to `HandleAi*` members (whitespace-only dedent), each replicating the chain's whole-handler `#if !WITH_EDITOR` EDITOR_ONLY stub; the shared `Result = McpHandlerUtils::CreateResultObject()` prologue moved into the 28 members that use it, and the per-branch feature ladders (MCP_HAS_STATE_TREE, MCP_HAS_SMART_OBJECTS, MCP_HAS_MASS_AI, MCP_HAS_ENVQUERY_TESTS, MCP_AI_HAS_BEHAVIOR_TREE_GRAPH) ride inside the extracted bodies unchanged. The 7 Behavior Tree graph actions became `HandleBehaviorTree*` members, each carrying the chain's BehaviorTreeEditor runtime module check (its `SubAction != TEXT("create")` disjunct resolved to the member's constant — false for create, true for the six graph members) and the six graph members replicating the chain's shared asset-load prologue (assetPath→behaviorTreePath→path fallback, BTGraph null-check, UpdateBehaviorTreeAsset + FindGraphNodeByIdOrName lambdas) plus its `#if WITH_EDITOR` / "Editor only." stub; the chain's payload-null precheck died (`FMcpCall::Execute` performs it). The 12 navigation actions are `HandleNavigation*` thin wrappers (level-structure precedent) over the untouched static free functions, replicating the chain's "Navigation operations require editor build" EDITOR_ONLY stub. The chains' `subAction` prologues, the three dispatcher declarations, the `MCP_REGISTER_HANDLER("manage_ai", ...)` registration (the last simple-macro registration in this batch), the now-unused McpConsolidatedActionRouting.h include in AIHandlers.cpp, and `McpDecl_ManageAi.h` all died with the conversion.
- **Manufactured subAction rewrites became typed calls (migration rule 5):** `create_behavior_tree` cloned the payload, rewrote `subAction` to the internal `create`, and re-entered `HandleBehaviorTreeAction` — its member keeps the payload clone + savePath-defaulting glue verbatim and calls `HandleBehaviorTreeCreate` directly; `add_composite_node`/`add_task_node` keep their enum→node-class mapping verbatim (Parallel reject, `BTTask_` prefixing, `Custom` nodeClass requirement) and call `HandleBehaviorTreeAddNode`; `add_decorator`/`add_service` split the four-way block's shared body per member — the `bDecorator`/`TypeParam`/`ClassPrefix` ternaries and `*SubAction` Printf echoes resolved to each action's constants (the only non-verbatim wrapper delta) — and call `HandleBehaviorTreeAddSubnode`. The internal `add_node`/`add_subnode` rewrite literals died with the chains; `create` survives only as the advertised action's own literal.
- **Hidden/dead deleted and ledgered (docs/dead-name-sweep-2026-07-04.md):** `set_ai_movement` (~154-line hidden CharacterMovement tuner, already ledgered — transport-dead, no decl row; advertise candidate parked for Aaron, recover from git) and the dispatcher's shadowed inline `create_nav_link_proxy` copy (below-radar dead duplicate: the top-of-function Navigation router always forwarded the advertised name to NavigationHandlers' point-link actor spawner, so the inline NavLinkProxy-*blueprint* creator was unreachable since the router landed). Hidden-entry counter 53 → 51.
- **Decl burn-down — 7 of 61 rows fixed, backed by the bodies' own rejects:** the six Behavior Tree graph rows (`add_node`, `add_subnode`, `break_connections`, `connect_nodes`, `remove_node`, `set_node_properties`) declared `assetPath` required although the retired chain resolves it through the behaviorTreePath/path fallbacks and joint-rejects only when all three are absent ("Missing 'assetPath' (or 'behaviorTreePath'/'path')") — assetPath optional now, at-least-one handler-enforced; `create_nav_link_proxy` dropped blueprintPath/name/path (read only by the deleted shadowed copy), its row now byte-identical to `create_smart_link`'s. The scoping sweep's one-of suspicion on both nav rows' location/startPoint/endPoint did NOT survive the body evidence — the live handler rejects when any of the three is missing, so all three stay required on both rows. The other 54 rows are byte-identical to the retired shim rows. Off-by-one reconciled: `McpTool_ManageAI.cpp`'s "60 actions" header was stale — `ManageAICore()` has 60 names and the schema union adds `add_subnode` (via `BehaviorTree()`), so the advertised surface and the decl count are 61; the header reads 61 now.
- Flags authored: `RequiresEditor` on all 61 (all three retired chains were whole-editor-gated; the members answer their stubs in non-editor builds); `Mutating` on everything except the readers — `get_ai_info`, `get_blackboard_value`, `get_navigation_info` — and `add_eqs_context` (honest NOT_IMPLEMENTED error, writes nothing). Gate battery: decl lint PASS, param reconciliation PASS, docs-reference PASS, per-file brace/paren/#if-#endif balance net 0, param-row byte-compare vs the retired shim rows (54 identical, 6 flips-only, 1 rewritten), member-body inverse byte-compare vs the retired branches (37 AI + 7 BT extracted verbatim, 12 Navigation wrapper delegations, shared BT prologue ×6). Build + live smoke pending the next editor session.
